...
<template>
	<div class="main" :style="{ height: $appHeight + 'px' }">
		...
		<div class="rule">
			<div>
				<small>
					<!-- <template v-if="langStr == 'zh1'">
		地址 |邮编：150-0031 | 日本东京都涩谷区樱丘12-8 	
		</template> -->
					<template v-if="langStr == 'jp'">
						所在地 |〒150-0031 | 東京都渋谷区桜ヶ丘 12-8
					</template>
					<template v-if="langStr == 'ko'">
						위치 | 우편번호: 150-0031 | 도쿄특별시 시부야구 사쿠라가오카 12-8
					</template>
					<template v-if="langStr == 'vi'">
						Vị trí | Mã bưu điện: 150-0031 | Số 12-8, Phường Sakuragaoka, Quận Shibuya, Thành phố Tokyo
					</template>
					<template v-if="langStr == 'en'">
						Location | Zip code: 150-0031 | 12-8, Sakuragaoka, Shibuya-ku, Tokyo
					</template>
					<template v-if="langStr == 'zh'">
						地址 |郵編：150-0031 | 日本東京都澀谷區櫻丘12-8
					</template>
				</small>
			</div>
		</div>
